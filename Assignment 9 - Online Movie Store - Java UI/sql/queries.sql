...
SELECT FirstName, LastName FROM orders o, user_acc u WHERE MovieID=2 AND u.Email = o.Email ORDER BY FirstName ASC;

--List Emails, Movie Name and Movie Rating of people who gave movies a rating of more than or equal to 5.
SELECT u.Email, m.MovieName, mr.NumericRating FROM user_acc u, writes w, movie_review mr, links_to lt, movie m WHERE mr.NumericRating >=5 AND u.Email = w.Email AND w.RevID = mr.RevID AND mr.RevID = lt.RevID AND lt.MovieID = m.MovieID ORDER BY mr.NumericRating DESC;

--List the Emails and Names of people who have subscribed to the Studio 'Walt Disney Animation'
SELECT u.Email, u.FirstName, u.LastName FROM user_acc u, subscribes_to st, Fanclub_membership fm, has h WHERE h.StudioName = 'Walt Disney Pictures' AND u.Email = st.Email AND st.MembershipID = fm.MembershipID AND fm.MembershipID = h.MembershipID ORDER BY Email ASC;


--Advanced Queries
--List MovieID, MovieName which can be purchased but not rented.
SELECT m.MovieID, m.MovieName FROM movie m, purchase p WHERE m.MovieID = p.MovieID AND m.MovieID NOT IN (SELECT  r.MovieID FROM rental r);
    
--List maximum, minimum, average price for renting a movie.
SELECT MIN(rentalprice), MAX(rentalprice), AVG(rentalprice) FROM rental;

--List user info whose payment method is only credit.
SELECT u.email, u.firstname, u.lastname FROM user_acc u WHERE NOT EXISTS (SELECT * FROM payment_method p, uses s WHERE u.email = s.email AND s.email = p.email AND p.methodtype = 'Debit');

--List movieID and name for PG-13 movies along with number of times it has been ordered.
SELECT o.movieID, m.movieName, count(*) FROM orders o, movie m WHERE o.movieId = m.movieID AND m.movieRating = 'PG-13' GROUP BY o.movieID, m.movieName ORDER BY movieID;

--List of movies produced by Walt Disney Pictures or have a resolution of 4K HDR.
SELECT m.movieName, m.resolution FROM movie m WHERE EXISTS (SELECT * FROM produces p WHERE m.movieId = p.movieId AND p.studioname = 'Walt Disney Pictures') UNION (SELECT m.movieName, m.resolution FROM movie m WHERE m.resolution = '4K HDR');